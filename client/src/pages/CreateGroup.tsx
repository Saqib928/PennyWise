...
export default function CreateGroup() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const available = results.filter(
            (u: User) => !selectedIds.has(u._id || u.id),
          );
  ...
  const handleCreate = async () => {
    ...
    try {
      const payload = {
        name: groupName,
        inviteUserIds: selectedUsers.map((u) => u._id || u.id || ""),
      };
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {selectedUsers.map((user) => (
                <div
                  ...
                  className="flex items-center gap-1 pl-3 pr-2 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-full text-xs font-medium animate-in zoom-in duration-200"
                >
                  <span className="truncate max-w-[100px]">{user.name}</span>
                  <button
                    onClick={() => removeUser(user._id || user.id!)}
        ...
        <div className="px-6 py-4 bg-white border-t border-gray-50 flex gap-3 sticky bottom-0 z-10">
          ...
          <button
            ...
            disabled={
              submitting || !groupName.trim() || selectedUsers.length === 0
            }
